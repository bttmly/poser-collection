...
var unique = require( "posable-array-uniq" )( Collection );

var fast = require( "./fast" )( Collection );

var cp = Collection.prototype;

var isArrayish = Array.isArray;
...
function baseFind ( arr, testFn ) {
  var i = -1;
  while ( ++i < arr.length ) {
    var test = !!testFn( arr[i], i, arr );
    if ( test ) {
...
var propLimit = (function () {
  function compareMax (prop, a, b) { return a[prop] > b[prop]; }
  function compareMin (prop, a, b) { return a[prop] < b[prop]; }
  return function (arr, prop, which) {
    var compare = which === 1 ? compareMax : compareMin;
    return cp.reduce.call( arr, function ( longest, item ) {
      if ( typeof item[prop] === "undefined" ) {
        return longest;
      }
      if ( ( longest === null ) || compare(prop, item, longest) ) { 
        return item;
      }
      return longest;
    }, null );
...
[ "forEach", "map", "reduce", "reduceRight", "filter", "some", "every", "indexOf", "lastIndexOf", "find", "findIndex", "fill" ].forEach( function( method ) {
...
cp.find = function ( testFn ) {
  return baseFind.call( null, this, testFn ).item;
};
...
cp.findIndex = function ( testFn ) {
  return baseFind.call( null, this, testFn ).index;
};
...
cp.zip = function () {
  ...
  // - we only need to factory() the longest
  return args
    .chainUnshift( this )
    .map( factoryOne )
    .sortBy( "length" )
...
cp.longest = function () {
  return propLimit( this, "length", 1 );
};

cp.shortest = function () {
  return propLimit( this, "length" );
};
